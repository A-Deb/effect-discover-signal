...
class Metrics(object):
    ...
    def base_criteria(self, g, w, thr1=0, thr2=7):
        ...
        :type thr2: int
        """
        at_least_thresh1_after = (datetime.strptime(g.reported, TIME_PATTERN) -
                                  datetime.strptime(w.submitted, TIME_PATTERN)).total_seconds() >= thr1
        no_more_than_thresh2_days = abs(datetime.strptime(w.occurred, TIME_PATTERN) -
                                        datetime.strptime(g.occurred, TIME_PATTERN)).total_seconds() / 86400 < thr2
    ...
    def occ_time_score(self, g, w, thresh=7):
        ...
        :type thresh: float
        """
        t1 = datetime.strptime(g.occurred, TIME_PATTERN)
        t2 = datetime.strptime(w.occurred, TIME_PATTERN)
    ...
    def event_details_score_mal_email(self, g, w):
        ...
        scores = list(filter(lambda x: x is not None, [sen, sub, max(lin, att)]))
    ...
    def lead_time_score(self, g, w):
        ...
        :type g: MetricGroundTruth
        """
        rep = datetime.strptime(g.reported, TIME_PATTERN)
        iss = datetime.strptime(w.submitted, TIME_PATTERN)
        diff = rep - iss
    ...
    def utility_time_score(self, g, w):
        ...
        :type g: MetricGroundTruth
        """
        occ = datetime.strptime(g.occurred, TIME_PATTERN)
        iss = datetime.strptime(w.submitted, TIME_PATTERN)
        diff = occ - iss
        return diff.total_seconds() / 86400
